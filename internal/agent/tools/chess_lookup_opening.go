...
// ecoOpenings là bảng khai cuộc tối giản, khóa là chuỗi nước SAN nối bằng dấu cách.
// Tra theo tiền tố dài nhất khớp. Có thể mở rộng dần.
...
func (t *ChessLookupOpeningTool) Execute(ctx context.Context, args json.RawMessage) (*types.ToolResult, error) {
	...
	// Tìm tiền tố dài nhất khớp bảng ECO (giới hạn ~16 nước đầu).
	limit := len(sans)
	if limit > 16 {
		limit = 16
	}
