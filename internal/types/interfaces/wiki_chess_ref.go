...
type WikiChessRefRepository interface {
	...
	// ReplaceForArticle thay toàn bộ tham chiếu cờ TỪ nội dung một bài viết
	// (nguồn article; kb_id rỗng, page_slug = slug bài viết — cùng mẫu ReplaceForLesson).
	ReplaceForArticle(ctx context.Context, tenantID uint64, articleSlug string, refs []types.WikiChessRef) error
	...
	DeleteForArticle(ctx context.Context, tenantID uint64, articleSlug string) error
