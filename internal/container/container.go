...
func BuildContainer(container *dig.Container) *dig.Container {
	...
	must(container.Provide(repository.NewChessCourseRepository))
	must(container.Provide(repository.NewChessLibraryRepository))
	must(container.Provide(repository.NewChessSlugAliasRepository))
	must(container.Provide(repository.NewChessKBIndexRepository))
	...
	must(container.Provide(repository.NewWikiChessRefRepository))
	...
	must(container.Provide(service.NewChessKnowledgeIndexer))
	must(container.Provide(service.NewChessCourseService))
	must(container.Provide(service.NewChessLibraryService))
	must(container.Provide(service.NewChessEngineService))
	...
	must(container.Provide(handler.NewChessCourseHandler))
	must(container.Provide(handler.NewChessLibraryHandler))
	must(container.Provide(handler.NewChessEngineHandler))
	must(container.Provide(handler.NewChessRefHandler))
